...
## 2026-03-24 会话（快捷键改为纯字母）

### 任务背景
将主题选择快捷键从"数字+字母"改为纯字母映射，简化快捷键逻辑。

### 已完成任务

#### Task 6.1: 修改快捷键映射常量
- [x] 修改 `src/hooks/useKeyboardShortcuts.ts`
- [x] SHORTCUT_KEYS 从 '1234567890QWERTY...' 改为 'QWERTY...'
- [x] 构建验证成功 (363.38 KiB)
- [x] 提交 (281f8e9)
...
| 文件 | 变更 |
|------|------|
| `src/hooks/useKeyboardShortcuts.ts` | 移除数字键，只保留字母键 |

### 完成标准
- [x] SHORTCUT_KEYS 常量已修改为纯字母
- [x] 字母键快捷键正常工作（支持26个分类）
- [x] 数字键不再触发分类选择

---

## 2026-03-24 会话（快捷键角标视觉优化）

### 任务背景
将快捷键角标从统一的黑色半透明背景改为使用对应分类的主题色背景，实现视觉融合。

### 已完成任务

#### Task 7.1: 修改角标样式
- [x] 修改 `CategoryList.tsx` 第72行
- [x] 移除 `bg-black/20` Tailwind 类
- [x] 移除 `text-white/60` 改为 `text-white`
- [x] 添加内联 `style={{ backgroundColor: `${category.color}99` }}`
- [x] 构建验证成功 (370.52 KiB)
- [x] 提交 (7396062)
...
| 文件 | 变更 |
|------|------|
| `src/components/CategoryList/CategoryList.tsx` | 角标背景改用分类主题色 |

### 技术说明
- `${category.color}99` 表示 60% 不透明度（99 是 HEX 透明度后缀）
- 例如 `#3B82F699` 表示蓝色 60% 不透明度
